...
const server=http.createServer(async(req,res)=>{
  try {
    ...
    const provider=route==='/kimi/v1/responses'?'kimi':route==='/custom/deepseek/v1/responses'?'deepseek':null;
    ...
    assert.ok(provider==='kimi'?body.model==='kimi-k3':body.model.startsWith('deepseek-v4-'));
