...
- [账号导入方式](#账号导入方式)
- [兼容矩阵](#兼容矩阵)
- [与 cc-router 的覆盖对比](#与-cc-router-的覆盖对比)
- [典型使用方式](#典型使用方式)
- [安装](#安装)
- [从源码运行](#从源码运行)
- [数据和安全边界](#数据和安全边界)
...
[finch-xu/cc-router](https://github.com/finch-xu/cc-router) 是 Claude Code 端（Anthropic Messages 协议）的同类工具，整理了一份较全的国内外 Coding Plan / API 清单。Codex Switcher 用 Codex `/v1/responses` ⇄ OpenAI `/chat/completions` 翻译，跟 cc-router 协议不同，所以**只覆盖能讲 OpenAI Chat Completions 的那部分**：

| cc-router provider | 我们 | base_url | 协议 | 备注 |
| --- | :---: | --- | --- | --- |
| anthropic | ⛔ | — | Anthropic only | 协议不兼容，需另写 Responses↔Anthropic 翻译 |
| zhipu (GLM 普通) | ✅ | `open.bigmodel.cn/api/paas/v4` | chat_completions | 智谱 GLM 普通端点 |
| zhipu (GLM Coding Plan) | ✅ | `open.bigmodel.cn/api/coding/paas/v4` | chat_completions | 已验证 |
| deepseek | ✅ | `api.deepseek.com/v1` | chat_completions | **已端到端验证**（v4-pro / v4-flash） |
| moonshot (Kimi) | ✅ | `api.moonshot.cn/v1` | chat_completions | preset 就位 |
| minimax | ✅ | `api.minimax.chat/v1` | chat_completions | preset 就位 |
| xiaomi (MiMo Token Plan) | ✅ | `token-plan-sgp.xiaomimimo.com/v1` | chat_completions | 已踩坑：reasoning_content 历史已加修剪 workaround |
| xiaomi (MiMo 按量付费) | ✅ | `api.xiaomimimo.com/v1` | chat_completions | sk-key，跟 Token Plan 区分 |
| alibaba (DashScope) | ✅ | `dashscope.aliyuncs.com/compatible-mode/v1` | chat_completions | 通义千问 |
| volcengine (火山方舟) | ✅ | `ark.cn-beijing.volces.com/api/v3` | chat_completions | model 字段需填 endpoint id |
| openrouter | ✅ | `openrouter.ai/api/v1` | chat_completions | 500+ 模型聚合 |
| tencent (混元) | ✅ | `api.hunyuan.cloud.tencent.com/v1` | chat_completions | preset 就位 |
| aiberm | ✅ | `aiberm.com/v1` | chat_completions | 双协议聚合，我们走 OpenAI 路径 |
| whatai (神马中转) | ✅ | `api.whatai.cc/v1` | chat_completions | 双协议聚合，我们走 OpenAI 路径 |
| ollama (本地) | ✅ | `localhost:11434/v1` | chat_completions | 本地推理，API Key 可填任意非空字符串 |
| fireworks | ✅ | `api.fireworks.ai/inference/v1` | chat_completions | 海外按量 / Fire Pass 订阅 |
| stepfun (阶跃星辰) | ✅ | `api.stepfun.com/v1` | chat_completions | preset 就位 |
| baidu (千帆 ERNIE) | ✅ | `qianfan.baidubce.com/v2` | chat_completions | preset 就位 |
| modelscope (魔搭) | ✅ | `api-inference.modelscope.cn/v1` | chat_completions | Qwen 系列为主 |
| ucloud (Modelverse) | ✅ | `deepseek.uk-tokyo.ucloud-global.com/v1` | chat_completions | 多模型聚合，海外区 |
| openai_codex (ChatGPT 订阅) | ✅ | — | Responses 原生 | OAuth 流程，不是 relay |
| kiro (Kiro IDE) | ⛔ | — | Anthropic only | 协议不兼容 |
| 自定义 | ✅ | 用户填 | 用户选 | — |

**额外内置（cc-router 没有）：**
- **通用 Responses 中转** — `relay_protocol = "responses"`，覆盖所有基于 new-api / sub2api / CLIProxyAPI 的第三方中转站（PinCC / PackyCode / AICodeMirror / 自建 CLIProxyAPI 等），用户填 base + key 就行。
- **MiMo 按量付费** — cc-router 把小米归为一条，我们拆出 Token Plan 和按量付费两条 endpoint。

**目前不能覆盖（pure Anthropic Messages 协议）：**
- `anthropic`（Anthropic 官方 API）
- `kiro`（Kiro IDE AWS 中转）

Codex 不讲 Anthropic Messages。要支持得另写一套 Responses ↔ Anthropic 翻译层（≈ Moon Bridge 工作量），暂未排上。
